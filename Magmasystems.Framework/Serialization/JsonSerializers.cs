...
namespace Magmasystems.Framework.Serialization
{
    [Flags]
    ...
    public static class Json
    {
        ...
        #region Some useful conversion functions
        public static object JArrayToList(object value)
        {
            if (value is JArray jArray && jArray?.Count > 0 && jArray[0] != null)
            {
                // Determine the underlying base type
                if (!(jArray[0] is JValue jValue))
                    return value;

                Type underlyingType = jValue.Value.GetType();

                // Create a C# generic list of that type
        ...
        public static object JArrayToList<T>(object value)
        {
            if (value is JArray jArray && jArray?.Count > 0 && jArray[0] != null)
            {
        ...
        public static object JArrayToArray<T>(object value)
        {
            if (value is JArray jArray && jArray?.Count > 0 && jArray[0] != null)
            {
                Type underlyingType = typeof(T);
                ...
                // Convert the JArray to a .NET generic list
                return ((List<T>)jArray.ToObject(listType)).ToArray();
            }
        ...
        {
            return value is JArray jArray && jArray?.Count > 0 && jArray[0] != null;
        }
        #endregion
    }
